...
w(r"src\buckler\__init__.py", '"""Buckler Boot Camp - real API + mock fallback"""\nfrom src.buckler.client import fetch_player_data\n')

w(r"src\plugins\stats.py", r'''"""SF6 stats command - auto-detects real API if cookie available"""
...
async def hsf6(event: MessageEvent, args: Message = CommandArg()):
    ...
    await sf6_cmd.send("Fetching SF6 data, please wait...")
    try:
        cached = await get_cached_stats(sid)
        if cached:
            data = PlayerData(**cached)
            await sf6_cmd.send("(Using cached data)")
        else:
...
# Cookie helper guide
w(r"data\buckler_cookie.txt", "# Buckler Cookie Configuration\n# 1. Open Chrome/Edge, go to https://www.streetfighter.com/6/buckler\n# 2. Login with your Capcom ID\n# 3. F12 -> Application -> Cookies -> www.streetfighter.com\n# 4. Copy all cookies as: name1=val1; name2=val2; ...\n# 5. Paste below (single line):\n")

print("Phase 3: plugin + init + cookie template done!")
